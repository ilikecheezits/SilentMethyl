#!/usr/bin/env python3
"""
Extracts regulatory annotations, epigenetic context, and JASPAR TF motif changes
for a specific variant (chr8:g.70148394G>A in NCOA2).

Run this script from the project root directory:
python scripts/14_get_application_info.py
"""
...
# Suppress minor warnings from BioPython/pandas for cleaner output
warnings.filterwarnings("ignore")

# --- Configuration ---
VARIANT_CHR = "chr8"
VARIANT_POS = 70148394  # 1-based coordinate
REF_ALLELE = "G"
ALT_ALLELE = "A"
WINDOW_SIZE = 25  # 25bp on each side (51bp total) to fit larger vertebrate motifs
...
def check_genomic_region(chrom, pos_1based):
    """
    Categorizes the variant into Promoter/TSS, UTR, Gene body, or Intergenic.
    This exactly mirrors the logic in 12_biological_context_analysis.py.
    """
    ...
    with gzip.open(GTF_PATH, "rt") as f:
        for line in f:
            ...
            # Filter for NCOA2 to speed up parsing for this specific script
            if "NCOA2" not in fields[8]: continue
            
            start, end = int(fields[3]), int(fields[4])
            strand, feature = fields[6], fields[2]
            
            # Original logic exactly as written in the biological context scripts
    ...
    # Exclusive Hierarchy exactly as in 12_biological_context_analysis.py
...
def get_epigenetic_quartiles_and_cpg(chrom, pos):
    """
    Computes ATAC and H3K27ac quartiles from test.csv and maps the CpG island status dynamically.
    """
...
def get_sequences():
    """Extracts reference sequence from FASTA and generates the alternate sequence."""
...
def scan_motifs(ref_seq, alt_seq):
    """Scans the sequences for vertebrate CORE motifs from JASPAR."""
