...
def evaluate(model, loader, device, amp_enabled):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="[TEST CONTEXT RC-AVERAGED]"):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate journal context-only model on the untouched chromosome-held-out test split")
    parser.add_argument("--test_path", default="data/datafiles/test.csv")
    parser.add_argument("--weights_path", default="checkpoints_journal/seed42/epi/best_weights.pth")
    parser.add_argument("--output_dir", default="results/journal/seed42/epi")
    ...
    model = EpigeneticOnlyModel(hidden_size=hidden_size, tabular_dim=9)
    ...
    save_standard_figures(predictions, metrics, out, "Context-only")

    logging.info("TEST context | n=%d | beta MAE %.5f RMSE %.5f | M MAE %.5f RMSE %.5f | AUC %.5f | RC beta MAE %.5f corr %.5f",
