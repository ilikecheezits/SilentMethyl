...
# =============================================================================
def main():
    logging.info("[*] Loading Data & Locating CHD5...")
    ...
    # =========================================================================
    # 5. Token to Base Pair Mapping (The Fix)
    # =========================================================================
    ...
    # Map every chunk's length back to its exact base pair boundaries
    ...
    # The Npas2 regulatory window in base pairs (41bp window from your JASPAR scan)
    zone_start_bp = max(0, mut_idx_in_1000 - 20)
    zone_end_bp = min(1000, mut_idx_in_1000 + 21)

    # =========================================================================
    # 6. Plotting with Accurate BP Coordinates
    # =========================================================================
    logging.info("[*] Plotting Verified Saliency Map with Physical BP Coordinates...")
    plt.figure(figsize=(14, 6))

    # Plot using exact widths and edge-alignment so it physically matches the genome
    plt.bar(bp_starts, valid_weights, width=token_widths, align='edge', color='#94a3b8', alpha=0.9, edgecolor='none')

    plt.axvspan(zone_start_bp, zone_end_bp, color='#fca5a5', alpha=0.4, label=f'{TARGET_TF} Motif Region')
    plt.axvline(mut_idx_in_1000, color='#ef4444', linewidth=2, linestyle='--', label='C>T Mutation Locus')
    ...
    plt.title(f'Neural Network Sequence Attention Profile for {TARGET_GENE}', fontsize=16, fontweight='bold')
    plt.xlabel('Genomic Coordinate (Base Pairs)', fontsize=12)
    plt.ylabel('Attention Pooling Weight (Importance Score)', fontsize=12)
    
    # Zoom in to properly show the 400bp to 600bp window centered perfectly around the 500bp mark
    zoom_margin = 200
    plt.xlim(max(0, mut_idx_in_1000 - zoom_margin), min(1000, mut_idx_in_1000 + zoom_margin))
    
    plt.legend(loc='upper right')
    plt.grid(True, linestyle=':', alpha=0.5)
    plt.tight_layout()

    plot_path = os.path.join(OUTPUT_DIR, 'chd5_saliency_map_verified_bp.png')
