#!/bin/bash
#SBATCH --job-name=Abl_Shape
#SBATCH --partition=GPU-shared
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=4
#SBATCH --gres=gpu:v100-32:1
#SBATCH --time=20:00:00
#SBATCH --output=ablation_shape_log_%j.txt
#SBATCH --error=ablation_shape_err_%j.txt
...
python scripts/01_train_abalations.py \
  ...
  --save_dir "checkpoints_ablation_shape_only" \
  --batch_size 4 \
  --grad_accum_steps 8 \
  --epochs 10 \
  --ablation_mode "no_epi"

echo "[✓] Shape Only Ablation Complete!"
