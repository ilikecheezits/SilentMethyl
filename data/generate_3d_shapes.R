...
data_dir <- file.path(script_dir, "datafiles")
...
extract_3d_tensor <- function(fasta_path, output_filename) {
    ...
    for (i in seq_along(SHAPE_TYPES)) {
...
if (file.exists(file.path(data_dir, "train_100bp.fasta"))) {
    extract_3d_tensor(file.path(data_dir, "train_100bp.fasta"), file.path(data_dir, "train_3d_shapes.tsv"))
    gc()
}

if (file.exists(file.path(data_dir, "val_100bp.fasta"))) {
    extract_3d_tensor(file.path(data_dir, "val_100bp.fasta"), file.path(data_dir, "val_3d_shapes.tsv"))
    gc()
}
if (file.exists(file.path(data_dir, "test_100bp.fasta"))) {
    extract_3d_tensor(file.path(data_dir, "test_100bp.fasta"), file.path(data_dir, "test_3d_shapes.tsv"))
    gc()
}
