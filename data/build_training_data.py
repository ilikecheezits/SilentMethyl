...
TRAIN_OUT = DATAFILES_DIR / "train.csv"
VAL_OUT = DATAFILES_DIR / "val.csv"
TEST_OUT = DATAFILES_DIR / "test.csv"
...
def write_fasta_file(df, output_path, seq_column="Healthy_5000bp_DNA"):
    ...
    with output_path.open("w") as handle:
        for _, row in df.iterrows():
            ...
            seq_window = str(row[seq_column])[2450:2550]
            handle.write(f"{header}\n{seq_window}\n")


def write_split_csvs(train_df, val_df, test_df):
...
def main():
    ...
    valid_mask = []
    num_faulty = 0
    for _, row in tqdm(df_manifest.iterrows(), total=len(df_manifest), desc="Extracting DNA"):
    ...
    for chunk in tqdm(
        pd.read_csv(
            ...
            chunksize=50000,
            dtype={col: "float32" for col in healthy_cols},
        ),
        desc="Processing methylation",
    ):
    ...
    bw_files = {name: pyBigWig.open(str(path)) for name, path in BW_PATHS.items()}

    new_features = {name: [] for name in BW_PATHS.keys()}

    for _, row in tqdm(df_master.iterrows(), total=len(df_master), desc="Mining BigWigs"):
        chrom = str(row["chr"])
        pos = int(row["pos"])
    ...
    print("[*] Writing CSV files and FASTA files...")
    train_df = sort_dataframe(train_df)
    val_df = sort_dataframe(val_df)
    test_df = sort_dataframe(test_df)

    write_split_csvs(train_df, val_df, test_df)
    write_fasta_file(train_df, DATAFILES_DIR / "train_100bp.fasta")
    write_fasta_file(val_df, DATAFILES_DIR / "val_100bp.fasta")
    write_fasta_file(test_df, DATAFILES_DIR / "test_100bp.fasta")

    print(f"[✓] ALL STEPS COMPLETE! Training data successfully built and split. Outputs saved to {DATAFILES_DIR}")
